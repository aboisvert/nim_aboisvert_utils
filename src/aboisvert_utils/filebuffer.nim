...
proc initFileBuffer*(file: File): FileBuffer =
  ## Initialize the
  result.file = file

proc asOpenArray*(cb: var FileBuffer): openArray[char] =
  cb.buffer.toOpenArray(cb.currentPos, cb.lastValidPos)

proc shift(cb: var FileBuffer, n: int) =
...
proc fill*(cb: var FileBuffer): int =
...
proc readLine*(cb: var FileBuffer, includeEol = false): tuple[line: openArray[char], EOF: bool] =
